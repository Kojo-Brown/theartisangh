...
- M5 ✅ Same `voice.transcribe` consumer now handles `kind=bookingRequest` jobs — writes transcript to the `Booking` row. `payment.reconcile` queue is registered for future async payout / refund retries; consumer to follow when payout moves off the request path.
